...
| `storage_backend` | `None` | Where benchmark run data (NPZ/JSON/CSV) is physically stored. Defaults to a `LocalFilesystemBackend` (cwd-relative). Swapping this redirects all benchmark artifacts (e.g. to a scratch disk or S3-backed prefix) without code changes. See [Storage & Checkpointing](Storage-and-Checkpointing.md). |
