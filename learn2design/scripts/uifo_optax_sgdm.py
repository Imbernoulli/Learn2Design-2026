"""Test script for OptaxSGDM optimizer."""

from dfbench.problems import UIFOProblem
from dfbench import Objective

from learn2design.example_algorithms import OptaxSGDM
...
optimizer.optimize(
    ...
    momentum=0.9,
    grad_clip_norm=1.0,
    random_seed=SEED,  # This is only for random param generation in OptaxSGDM
)

# Save run data
obj.save_run_data(hyper_param_str="lr0.1_mom0.9")  # Optional hyperparameter string will be included in the filename
# Output loss plot, sensitivity plot, final parameters(JSON), and loss history(JSON) which can be toggled by arguments
obj.output_to_files(hyper_param_str="lr0.1_mom0.9")   
